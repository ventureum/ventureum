...
const TokenSale = artifacts.require("modules/TokenSale");
const TokenCollectorModule = artifacts.require("modules/TokenCollector");
...
const ACLHandler  = artifacts.require(
    "handlers/ACLHandler");
...
const PROJECT_CONTROLLER_STORAGE_CI = Web3.utils.keccak256(
...
const REGISTER_SIG = wweb3.eth.abi.encodeFunctionSignature(
...
contract("TokenSaleTest", function ([root, _, purchaser]) {
    before(async function () {
        ...
        //deploy modules
        this.tokenCollector = await TokenCollectorModule.new(
            this.kernel.address);
        this.tokenSaleModule = await TokenSale.new(this.kernel.address);
        ...
        this.aclHandler.permit(
            TOKEN_SALE_CI,
            TOKEN_COLLECTOR_CI,
            WITHDRAW_SIG).should.be.fulfilled;
    ...
    describe("basic test", function () {
        ...
        it("should receive correct handler", async function () {
            let result = await this.tokenSaleModule.handlers.call(
                ACL_HANDLER_CI);
            result.should.be.equal(this.aclHandler.address);
            result = await this.tokenSaleModule.handlers.call(
                CONTRACT_ADDRESS_HANDLER_CI);
    ...
    describe("basic functional test", function () {
        it("should start token sale", async function () {
            this.projectController.registerProject(
                PROJECT_CI1, root, this.token.address).should.be.fulfilled;
        ...
        it("should finalize success", async function () {
            ...
            this.projectController.registerProject(
                testCI1, root, this.token.address).should.be.fulfilled;

            await this.tokenSaleModule.startTokenSale(
                testCI1, this.rate, this.token.address).should.be.fulfilled;
            await this.tokenSaleModule.finalize(testCI1).should.be.fulfilled;
            await this.tokenSaleModule.buyTokens(
                testCI1, {value: this.ethAmount, from: purchaser})
        ...
        it("should rejected cause project already finalized", async function () {
            ...
            this.projectController.registerProject(
                testCI2, root, this.token.address).should.be.fulfilled;

            await this.tokenSaleModule.startTokenSale(
                testCI2, this.rate, this.token.address).should.be.fulfilled;
            await this.tokenSaleModule.finalize(testCI2).should.be.fulfilled;
            await this.tokenSaleModule.buyTokens(
                testCI2, {value: this.ethAmount, from: purchaser})
                    .should.be.rejectedWith(EVMRevert);
        });
        ...
        it("should rejected cause finalize a finalized project",
            ...
            this.projectController.registerProject(
                testCI3, root, this.token.address).should.be.fulfilled;

            await this.tokenSaleModule.startTokenSale(
                testCI3, this.rate, this.token.address).should.be.fulfilled;
            await this.tokenSaleModule.finalize(testCI3).should.be.fulfilled;
            await this.tokenSaleModule.finalize(testCI3)
                .should.be.rejectedWith(EVMRevert);
        });
    });
    ...
    describe("advanced testing include branch test", function () {
        before(async function () {
            ...
            await this.tokenCollector.deposit(
                this.token.address, this.depositValue).should.be.fulfilled;
        })

        it("should buy token success", async function () {
            const PROJECT_CI = Web3.utils.keccak256("advanced1");
            this.projectController.registerProject(
                PROJECT_CI, root, this.token.address).should.be.fulfilled;

            await this.tokenSaleModule.startTokenSale(
                PROJECT_CI, this.rate, this.token.address).should.be.fulfilled;

            const tokenAmount = this.rate * this.ethAmount;

            const preStoreTokenBalance =
                await this.tokenCollector.balanceOf.call(
                    this.token.address);
            const prePurchaserTokenBalance = await this.token.balanceOf(
                purchaser);
        ...
        it("should receive avg price equal rate", async function () {
            ...
            await this.tokenSaleModule.avgPrice.call(PROJECT_CI)
                .should.be.rejectedWith(EVMRevert);
            await this.tokenSaleModule.buyTokens(
                PROJECT_CI,
                {value: this.ethAmount, from: purchaser}).should.be.fulfilled;
            ...
            const afterFinalizedAvgPrice = await this.tokenSaleModule
                .avgPrice.call(PROJECT_CI);

            const randomCI = ROOT_CI;
            await this.tokenSaleModule.avgPrice.call(randomCI)
        ...
        it("should rejected cause project already exist", async function () {
            ...
            await this.tokenSaleModule.startTokenSale(
                PROJECT_CI, 123, this.token.address)
                    .should.be.rejectedWith(EVMRevert);
        });
        ...
        it("should rejected cause project not exist", async function () {
            ...
            await this.tokenSaleModule.startTokenSale(
                PROJECT_CI, this.rate, this.token.address)
                    .should.be.fulfilled;

            await this.tokenSaleModule.buyTokens(
                ROOT_CI, {value: this.ethAmount, from: purchaser})
                    .should.be.rejectedWith(EVMRevert);
        });
        ...
        it("should rejected cause buy too large token", async function () {
            ...
            await this.tokenCollector.withdraw(
                this.token.address, root, storeBalance);
            const preStoreTokenBalance = await this.tokenCollector
                .balanceOf.call(this.token.address);
            preStoreTokenBalance.should.be.bignumber.equal(0);
            await this.tokenSaleModule.buyTokens(
                PROJECT_CI,
                {value: this.ethAmount, from: purchaser})
                .should.be.rejectedWith(EVMRevert);
        });
        ...
        it("should rejected cause finalize a not exist project",
            ...
            await this.tokenSaleModule.finalize(ROOT_CI)
                .should.be.rejectedWith(EVMRevert);
        });

        it("should rejected cause CAH do not have project controller",
            async function () {
            await this.contractAddressHandler.unregisterContract(
                PROJECT_CONTROLLER_CI).should.be.fulfilled;
            await this.tokenSaleModule.finalize(ROOT_CI)
                .should.be.rejectedWith(EVMRevert);
